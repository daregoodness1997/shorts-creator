...
- **📂 Video Library**: Browse and select from videos in your `videos/` folder
- **🔢 Multiple Shorts**: Generate multiple shorts from a single video (1-5+ per video)
- **🎤 GPU-Accelerated Transcription**: CUDA-enabled Whisper for fast speech-to-text
- **🤖 AI Highlight Selection**: Gemini/GPT automatically finds the most engaging 2-minute segments
- **✅ LLM Provider Choice**: Use Gemini (free tier) or OpenAI GPT models
- **📝 Auto Subtitles**: Modern white text with bold styling and black outline for maximum readability
- **🎯 Smart Cropping**: 
...
- **⚙️ Automation Ready**: CLI arguments, auto-quality selection, batch processing
- **🔄 Concurrent Execution**: Unique session IDs allow multiple instances to run simultaneously
- **📦 Clean Output**: Slugified filenames (e.g., `my-video-title_short_1.mp4`) and automatic temp file cleanup
